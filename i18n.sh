...
for((j=0;j<${#source[*]};j++)) do
  if [ ! -d $project_dir/app/admin/locale/lang/trans ];then
    mkdir -p $project_dir/app/admin/locale/lang/trans
  fi
  cp -f ${source[j]} $project_dir/app/admin/locale/lang/trans/${target[j]}
done
