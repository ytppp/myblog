...
for((j=0;j<${#source[*]};j++)) do
  if [ ! -d $project_dir/app/frontend/locale/lang/trans ];then
    mkdir -p $project_dir/app/frontend/locale/lang/trans
  fi
  cp -f ${source[j]} $project_dir/app/frontend/locale/lang/trans/${target[j]}
done
beautify_echo "complete copy all i18n files..." success
